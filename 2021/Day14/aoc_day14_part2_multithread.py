...
def count_insertions(pair, step=1):
    p1, p2 = pair
...
list_pairs = [(polymer[i], polymer[i + 1]) for i in range(len(polymer) - 1)]
pool_count = len(polymer) - 1
pool_count = 4
i = 0
with Pool(pool_count) as p:
    print(f'process {p}')
    count_mol_list = p.map(count_insertions, list_pairs[i])
    i += 1

count_mol = init_count
for c_mol in count_mol_list:
    for key, value in c_mol:
